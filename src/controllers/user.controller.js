...
const loginUser = asyncHandler(async (req,res)=>{
    ...
    const options = {
        httpOnly: true, // after this client side js cannot access the cookie not modifiable
...
const upateAccountDetails = asyncHandler(async (req,res)=>{
    ...
    if(!(fullName || email)){
        throw new ApiError(400, "Full name or email is required to update account details");
    }
    User.findByIdAndUpdate(
        req.user?._id,
        {
...
const getUserChannelprofile = asyncHandler(async(req,res)=>{
    ...
    const channel = await User.aggregate([
        ...
        {
            $project:{ //To get only selected values
                fullName: 1,
                username: 1,
                subscribersCount: 1,
